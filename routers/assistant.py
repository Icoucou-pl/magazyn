...
from services.usage import get_stats, add_topup
...
def _is_super(user: CurrentUser) -> bool:
    """Super-admin rozpoznawany po emailu (jak w audit logu)."""
    se = settings.SUPER_ADMIN_EMAIL.strip().lower()
    return bool(se) and user.email.lower() == se
...
async def usage_stats(
    ...
    """Saldo + rozkład input/output + ostatnie zapytania (dla panelu licznika).
    Treść pytań widzi tylko super-admin — dla zwykłego admina jest wycinana."""
    return await get_stats(db, settings.STARTING_BALANCE_USD, include_query=_is_super(user))


@router.post("/usage/topup")
async def usage_topup(
    body: TopUpBody,
    db: AsyncSession = Depends(get_db),
    user: CurrentUser = Depends(get_current_user),
):
    """Rejestruje dopłatę do konta LLM. Tylko super-admin."""
    if not _is_super(user):
        raise HTTPException(403, "Dopłaty może rejestrować tylko super-administrator.")
    if body.amount <= 0:
        raise HTTPException(400, "Kwota dopłaty musi być dodatnia.")
    await add_topup(db, amount=body.amount, note=body.note, added_by=user.email)
    return await get_stats(db, settings.STARTING_BALANCE_USD, include_query=True)
