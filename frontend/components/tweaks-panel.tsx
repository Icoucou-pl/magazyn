...
// ============================================================
// MAGAZYN — Panel ustawień (motyw / akcent / gęstość)
// Konwersja tweaks-panel.jsx → .tsx (etap 0.3).
//
// CO ZOSTAJE 1:1: wygląd panelu i wszystkie kontrolki (.twk-* + komponenty
//   TweakSlider/Toggle/Radio/Select/Color/Number/Text/Section/Row).
//
// CO ZNIKA (bo to plumbing prototypów, nie zadziała na Railway):
//   - protokół hosta: postMessage __activate/__deactivate/__edit_mode_*, blok EDITMODE
//   - integracja z <deck-stage>
//   Zamiast tego: persystencja w localStorage + nakładanie motywu na <html>.
//
// CO DOCHODZI:
//   - mały trigger "⚙" (host już go nie poda); panel może też być sterowany
//     z zewnątrz przez propsy open/onClose (header podepniemy w 0.4).
//
// Domyślny eksport <Tweaks/> = gotowy panel apki (motyw, akcent, gęstość).
// Reguły [data-theme="light"] / [data-density="..."] żyją w globals.css (scalamy 0.4).
// ============================================================

import React from "react";

// ── Style (1:1 z oryginału; dodany tylko .twk-fab dla triggera) ──
const __TWEAKS_STYLE = `
  .twk-fab{position:fixed;right:16px;bottom:16px;z-index:2147483645;
    width:40px;height:40px;border-radius:999px;border:.5px solid rgba(255,255,255,.6);
    background:rgba(250,249,247,.78);color:#29261b;font-size:18px;line-height:1;cursor:pointer;
...
type SegOption = Primitive | { value: Primitive; label: string };
type ColorOption = string | string[];

// ── useTweaks — źródło prawdy + persystencja w localStorage ──
// setTweak('key', value) ALBO setTweak({ key: value, ... }).
export function useTweaks<T extends Record<string, unknown>>(
  ...
  // Hydratacja z localStorage po montażu (unika niezgodności SSR↔klient).
  React.useEffect(() => {
    try {
      const raw = localStorage.getItem(storageKey);
      if (raw) setValues((prev) => ({ ...prev, ...JSON.parse(raw) }));
    } catch { /* localStorage niedostępny — zostają defaults */ }
  }, [storageKey]);
...
// ── TweaksPanel — pływająca powłoka (sterowana lub samodzielna) ──
export function TweaksPanel({
  ...
  const close = () => { if (controlled) onClose?.(); else setOpenState(false); };
  ...
  return (
    <>
      ...
      {!controlled && !open && (
        <button className="twk-fab" aria-label="Ustawienia"
                onClick={() => setOpenState(true)}>⚙</button>
      )}
      {open && (
        <div ref={dragRef} className="twk-panel"
          ...
          <div className="twk-hd" onMouseDown={onDragStart}>
            <b>{title}</b>
            <button className="twk-x" aria-label="Zamknij"
...
export function TweakRadio({
  ...
  label: string; value: Primitive; options: SegOption[]; onChange: (v: Primitive) => void;
}) {
  ...
  if (!fitsAsSegments) {
    const resolve = (s: string): Primitive => {
  ...
  const segAt = (clientX: number): Primitive => {
...
// Kontrast względny — ptaszek nad próbką ma być czytelny na #111 i #fafafa.
