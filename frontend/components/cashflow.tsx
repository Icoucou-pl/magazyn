...
function CashflowView({ onContainerClick, initialTab }: {
  ...
  return (
    <div className="fade-in" style={{ display: "flex", flexDirection: "column", gap: 14, paddingBottom: 80 }}>
      ...
      <div style={noteStyle}>
        {tab === "money"
          ? <><b>Konto i pożyczki</b> — ręczne wpisy, osobno dla każdej spółki. <b>Stan konta</b> to odczyt podany przez księgową; jedna data = jeden wpis, powtórny zapis nadpisuje poprzedni. <b>Pożyczki</b> to wpłaty wspólników, <b>spłaty</b> to zwroty — w obu formularzach wpisujesz kwotę dodatnią, a tabela „Do spłaty wspólnikom” odejmuje jedno od drugiego. Na linię konta pożyczki działają <b>narastająco</b>, od dnia wpłaty w przód; kolumna „bez pożyczek” pokazuje, ile firma miałaby z własnej sprzedaży.</>
          : tab === "due"
    ...
    <>
      ...
      {agg.months.length > 0 && (
        <Card>
          ...
          {/* Wiersz płatności na wąskim ekranie: siatka 5 kolumn (producent | typ | data | waluta |
              kwota) ma minimum ~430 px z samych „auto" — na telefonie kolumny nachodziły na siebie.
              Rozbijamy na linie: identyfikacja, chip zaliczek, a niżej typ + data + kwoty. */}
          <style>{`
            @media (max-width: 720px) {
              .cf-drill { padding: 6px 12px 12px 14px !important; }
              .cf-row { display: flex !important; flex-wrap: wrap; align-items: center; gap: 6px 8px !important; }
              .cf-id { flex: 1 1 100%; flex-wrap: wrap; }
              .cf-zal { flex: 1 1 100%; margin-left: 0 !important; padding-left: 0 !important; }
              .cf-type { order: 2; }
              .cf-date { order: 3; }
              .cf-orig { order: 4; min-width: 0 !important; text-align: left !important; }
              .cf-orig:empty { display: none; }
              .cf-amt { order: 5; margin-left: auto; min-width: 0 !important; }
              .cf-month { gap: 8px !important; padding: 12px 12px !important; }
              .cf-month-label { width: auto !important; }
            }
          `}</style>
        </Card>
      )}
...
function MonthRow({ month: m, maxTotal, cur, rt, shop, hoveredMfr, colorOf, nameOf, isLast, onContainerClick }: {
  ...
  return (
    <div style={{ borderBottom: isLast ? "none" : "1px solid var(--border-soft)" }}>
      <div onClick={() => setOpen(!open)} className="cf-month" style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 18px", cursor: "pointer" }}
        ...
        <div className="cf-month-label" style={{ width: 100, flexShrink: 0, fontSize: 13, fontWeight: 600 }}>{m.label}</div>
      ...
      {open && (
        <div className="fade-in cf-drill" style={{ background: "var(--bg-elevated)", padding: "6px 18px 14px 44px", display: "flex", flexDirection: "column", gap: 6 }}>
...
function PayRow({ e, cur, rt, shop, onContainerClick }: {
  ...
  return (
    <div onClick={() => onContainerClick?.(e.id)} className="cf-row" style={{
      ...
      <div className="cf-id" style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
        ...
        {zaliczki.length > 0 && (
          <span className="cf-zal" style={{ display: "inline-flex", alignItems: "center", gap: 6, marginLeft: "auto", paddingLeft: 8, flexShrink: 0 }}>
            {zaliczki.map((z, zi) => (
      ...
      <span className="cf-type" style={{ fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 99, letterSpacing: "0.03em", whiteSpace: "nowrap",
        background: isZal ? "var(--info-soft)" : "var(--accent-soft)", color: isZal ? "var(--info)" : "var(--accent)" }}>
        {isZal ? "ZALICZKA" : "BALANCE"}
      </span>
      <span className="cf-date" style={{ fontSize: 12, color: dateColor, whiteSpace: "nowrap" }}>
        {dateTxt}{e.status === "plan" ? " ⏳" : ""}
      </span>
      <span className="num cf-orig" style={{ fontSize: 12, color: "var(--text-lo)", textAlign: "right", whiteSpace: "nowrap", minWidth: 70 }}>
        {cur === "PLN" ? fmtCur(e.kwota, e.waluta) : ""}
      </span>
      <span className="num cf-amt" style={{ fontSize: 13, fontWeight: 600, textAlign: "right", whiteSpace: "nowrap", minWidth: 96, color: est ? "var(--text-lo)" : "var(--text-hi)" }}>
